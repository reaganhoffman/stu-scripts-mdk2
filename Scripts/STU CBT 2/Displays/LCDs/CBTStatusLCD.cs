...
namespace IngameScript {
    partial class Program {
        public partial class CBTStatusLCD : STUDisplay {
            ...
            {
                Color[] _colors = new Color[CBT.PCM.PowerGroups.Length];
                for (int i = 0; i < CBT.PCM.PowerGroups.Length; i++)
                {
                    if (CBT.PCM.PowerGroups[i].Enabled) { _colors[i] = Color.Green; }
            ...
            public void BuildScreen(MySpriteDrawFrame frame, Vector2 centerPos, float scale = 1f) {
                ...
                for (int i = 0; i < CBT.PCM.PowerGroups.Length; i++)
                {
                    frame.Add(BuildTextSprite($"{CBT.PCM.PowerGroups[i].Name.Substring(0, 1)}", ((i % 4)+1) * GetTextSpriteWidth("AAA"), CharHeight * (7 + (float)Math.Floor(i/4f)), PowerStateColors[i]));
                }
            }
